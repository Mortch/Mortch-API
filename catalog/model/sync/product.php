...
class Product extends \Opencart\System\Engine\Model {
    ...
    public function getProductByProductId(int $product_id): array {
        $product_data = [];
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product` `p` WHERE `p`.`product_id` = '" . $product_id . "';");
     ...
     *
     * $product_id = $this->model_sync_product->addProduct($product_data);
     */
    public function addProduct(array $data): int {
        $this->db->query("INSERT INTO `" . DB_PREFIX . "product` SET `product_id` = '" . (int)$data['product_id'] . "',`master_id` = '" . (int)$data['master_id'] . "', `model` = '" . $this->db->escape((string)$data['model']) . "', `location` = '" . $this->db->escape((string)$data['location']) . "', `variant` = '" . $this->db->escape(!empty($data['variant']) ? json_encode($data['variant']) : '') . "', `override` = '" . $this->db->escape(!empty($data['override']) ? json_encode($data['override']) : '') . "',  `minimum` = '" . (int)$data['minimum'] . "', `subtract` = '" . (isset($data['subtract']) ? (bool)$data['subtract'] : 0) . "', `stock_status_id` = '" . (int)$data['stock_status_id'] . "', `date_available` = '" . $this->db->escape((string)$data['date_available']) . "', `manufacturer_id` = '" . (int)$data['manufacturer_id'] . "', `shipping` = '" . (isset($data['shipping']) ? (bool)$data['shipping'] : 0) . "', `points` = '" . (int)$data['points'] . "', `weight` = '" . (float)$data['weight'] . "', `weight_class_id` = '" . (int)$data['weight_class_id'] . "', `length` = '" . (float)$data['length'] . "', `width` = '" . (float)$data['width'] . "', `height` = '" . (float)$data['height'] . "', `length_class_id` = '" . (int)$data['length_class_id'] . "', `status` = '" . (bool)($data['status'] ?? 0) . "', `tax_class_id` = '" . (int)$data['tax_class_id'] . "',`erp_status` = '" . (int)$data['erp_status'] . "', `erp_stock_status_id` = '" . (int)$data['erp_stock_status_id'] . "', `erp_exist_status` = '" . (int)$data['erp_exist_status'] . "', `erp_exist_stock_status_id` = '" . (int)$data['erp_exist_stock_status_id'] . "', `sort_order` = '" . (int)$data['sort_order'] . "', `date_added` = NOW(), `date_modified` = NOW()");
        ...
        foreach ($data['product_description'] as $language_id => $value) {
            $this->model_sync_product->addDescription($product_id, $language_id, $value);
        }

        // Code
        if (isset($data['product_code'])) {
            foreach ($data['product_code'] as $product_code) {
                $this->model_sync_product->addCode($product_id, $product_code);
        ...
        if (isset($data['product_category'])) {
            foreach ($data['product_category'] as $category_id) {
                $this->model_sync_product->addCategory($product_id, $category_id);
        ...
        if (isset($data['product_filter'])) {
            foreach ($data['product_filter'] as $filter_id) {
                $this->model_sync_product->addFilter($product_id, $filter_id);
            }
        }
        ...
        if (isset($data['product_store'])) {
            foreach ($stores as $store) {
                if (!in_array($store['store_id'], $data['product_store'])) {
                    array_push($data['product_store'], $store['store_id']);
                }
            }
            foreach ($data['product_store'] as $store_id) {
                $this->model_sync_product->addStore($product_id, $store_id);
            }
        }
        ...
        if (isset($data['product_download'])) {
            foreach ($data['product_download'] as $download_id) {
                $this->model_sync_product->addDownload($product_id, $download_id);
            }
        }
        ...
        if (isset($data['product_related'])) {
            foreach ($data['product_related'] as $related_id) {
                $this->model_sync_product->addRelated($product_id, $related_id);
            }
        }
        ...
        if (isset($data['product_attribute'])) {
            foreach ($data['product_attribute'] as $product_attribute) {
                if ($product_attribute['attribute_id']) {
                    // Removes duplicates
                    $this->model_sync_product->deleteAttributes($product_id, $product_attribute['attribute_id']);

                    foreach ($product_attribute['product_attribute_description'] as $language_id => $product_attribute_description) {
                        $this->model_sync_product->addAttribute($product_id, $product_attribute['attribute_id'], $language_id, $product_attribute_description);
                    }
                }
            }
        }
        ...
        if (isset($data['product_option'])) {
            foreach ($data['product_option'] as $product_option) {
                $this->model_sync_product->addOption($product_id, $product_option);
            }
        }
        ...
        if (isset($data['product_subscription'])) {
            foreach ($data['product_subscription'] as $product_subscription) {
                $this->model_sync_product->addSubscription($product_id, $product_subscription);
            }
        }
        ...
        if (isset($data['product_discount'])) {
            foreach ($data['product_discount'] as $product_discount) {
                $this->model_sync_product->addDiscount($product_id, $product_discount);
            }
        }
        ...
        if (isset($data['product_image'])) {
            foreach ($data['product_image'] as $product_image) {
                $this->model_sync_product->addImage($product_id, $product_image);
        ...
        if (isset($data['product_reward'])) {
            foreach ($data['product_reward'] as $customer_group_id => $product_reward) {
                if ((int)$product_reward['points'] > 0) {
                    $this->model_sync_product->addReward($product_id, $customer_group_id, $product_reward);
                }
            }
        }
            ...
            foreach ($data['product_seo_url'] as $store_id => $language) {
                foreach ($language as $language_id => $keyword) {
                    if (in_array($language_id, $language_list)){
                        ...
                        foreach ($stores as $store){
                            $this->model_design_seo_url->addSeoUrl('product_id', $product_id, $keyword, $store['store_id'], $language_id);
                        }
                    }
        ...
        if (isset($data['product_layout'])) {
            foreach ($data['product_layout'] as $store_id => $layout_id) {
                if ($layout_id) {
                    $this->model_sync_product->addLayout($product_id, $store_id, $layout_id);
                }
            }
        }
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->editProduct($product_id, $product_data);
     */
    public function editProduct(int $product_id, array $data): void {
        $this->db->query("UPDATE `" . DB_PREFIX . "product` SET `erp_status` = '" . (int)$data['erp_status'] . "', `erp_stock_status_id` = '" . (int)$data['erp_stock_status_id'] . "', `erp_exist_status` = '" . (int)$data['erp_exist_status'] . "', `erp_exist_stock_status_id` = '" . (int)$data['erp_exist_stock_status_id'] . "', `model` = '" . $this->db->escape((string)$data['model']) . "', `location` = '" . $this->db->escape((string)$data['location']) . "', `variant` = '" . $this->db->escape(!empty($data['variant']) ? json_encode($data['variant']) : '') . "', `override` = '" . $this->db->escape(!empty($data['override']) ? json_encode($data['override']) : '') . "',  `minimum` = '" . (int)$data['minimum'] . "', `subtract` = '" . (isset($data['subtract']) ? (bool)$data['subtract'] : 0) . "', `stock_status_id` = '" . (int)$data['stock_status_id'] . "', `image` = '" . $this->db->escape((string)$data['image']) . "', `date_available` = '" . $this->db->escape((string)$data['date_available']) . "', `manufacturer_id` = '" . (int)$data['manufacturer_id'] . "', `shipping` = '" . (isset($data['shipping']) ? (bool)$data['shipping'] : 0) . "',  `points` = '" . (int)$data['points'] . "', `weight` = '" . (float)$data['weight'] . "', `weight_class_id` = '" . (int)$data['weight_class_id'] . "', `length` = '" . (float)$data['length'] . "', `width` = '" . (float)$data['width'] . "', `height` = '" . (float)$data['height'] . "', `length_class_id` = '" . (int)$data['length_class_id'] . "', `status` = '" . (bool)($data['status'] ?? 0) . "', `tax_class_id` = '" . (int)$data['tax_class_id'] . "', `sort_order` = '" . (int)$data['sort_order'] . "', `date_modified` = NOW() WHERE `product_id` = '" . (int)$product_id . "'");

        // Description
        $this->model_sync_product->deleteDescriptions($product_id);
        ...
        // Code
        $this->model_sync_product->deleteCodes($product_id);
        ...
        // Categories
        $this->model_sync_product->deleteCategories($product_id);
        ...
        // Filters
        $this->model_sync_product->deleteFilters($product_id);
        ...
        // Stores
        $this->model_sync_product->deleteStores($product_id);
        ...
        // Downloads
        $this->model_sync_product->deleteDownloads($product_id);
        ...
        // Related
        $this->model_sync_product->deleteRelated($product_id);
        ...
        // Attributes
        $this->model_sync_product->deleteAttributes($product_id);
        ...
        // Options
        $this->model_sync_product->deleteOptions($product_id);
        ...
        // Subscription
        $this->model_sync_product->deleteSubscriptions($product_id);
        ...
        // Discounts
        $this->model_sync_product->deleteDiscounts($product_id);
        ...
        // Images
        $this->model_sync_product->deleteImages($product_id);
        ...
        // Rewards
        $this->model_sync_product->deleteRewards($product_id);

        if (isset($data['product_reward'])) {
            foreach ($data['product_reward'] as $customer_group_id => $value) {
                if ((int)$value['points'] > 0) {
                    $this->model_sync_product->addReward($product_id, $customer_group_id, $value);
                }
            }
        }
        ...
        // Layout
        $this->model_sync_product->deleteLayouts($product_id);
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->copyProduct($product_id);
     */
    public function copyProduct(int $product_id): int {
        $new_product_id = 0;

        $product_info = $this->model_sync_product->getProduct($product_id);
        ...
        if ($product_info) {
            ...
            $product_data['product_attribute'] = $this->model_sync_product->getAttributes($product_id);
            $product_data['product_code'] = $this->model_sync_product->getCodes($product_id);
            $product_data['product_category'] = $this->model_sync_product->getCategories($product_id);
            $product_data['product_description'] = $this->model_sync_product->getDescriptions($product_id);
            $product_data['product_discount'] = $this->model_sync_product->getDiscounts($product_id);
            $product_data['product_download'] = $this->model_sync_product->getDownloads($product_id);
            $product_data['product_filter'] = $this->model_sync_product->getFilters($product_id);
            $product_data['product_image'] = $this->model_sync_product->getImages($product_id);
            $product_data['product_layout'] = $this->model_sync_product->getLayouts($product_id);
            $product_data['product_option'] = $this->model_sync_product->getOptions($product_id);
            $product_data['product_subscription'] = $this->model_sync_product->getSubscriptions($product_id);
            $product_data['product_related'] = $this->model_sync_product->getRelated($product_id);
            $product_data['product_reward'] = $this->model_sync_product->getRewards($product_id);
            $product_data['product_store'] = $this->model_sync_product->getStores($product_id);
            ...
            $new_product_id = $this->model_sync_product->addProduct($product_data);
        }
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteProduct($product_id);
     */
    public function deleteProduct(int $product_id): void {
        ...
        $this->model_sync_product->deleteOptions($product_id);
        $this->model_sync_product->deleteRelated($product_id);
        $this->model_sync_product->deleteReports($product_id);
        $this->model_sync_product->deleteRewards($product_id);
        $this->model_sync_product->deleteStores($product_id);
        $this->model_sync_product->deleteSubscriptions($product_id);

        // Review
        $this->load->model('sync/review');

        $this->model_sync_review->deleteReviewsByProductId($product_id);
        ...
        // Coupon
        $this->load->model('sync/coupon');

        $this->model_sync_coupon->deleteProductsByProductId($product_id);

        $this->model_sync_product->editMasterId($product_id, 0);
     ...
     * $this->load->model('catalog/product');
     *
     * $product_id = $this->model_sync_product->addVariant($master_id, $data);
     */
    public function addVariant(int $master_id, array $data): int {
        $product_data = [];

        // Use master values to override the values
        $master_info = $this->model_sync_product->getProduct($master_id);
        ...
        if ($master_info) {
            ...
            // Descriptions
            $product_descriptions = $this->model_sync_product->getDescriptions($master_id);
            ...
            if (!isset($override['product_attribute'])) {
                $product_data['product_attribute'] = $this->model_sync_product->getAttributes($master_id);
            }

            // Category
            if (!isset($override['product_category'])) {
                $product_data['product_category'] = $this->model_sync_product->getCategories($master_id);
            }

            // Discounts
            if (!isset($override['product_discount'])) {
                $product_data['product_discount'] = $this->model_sync_product->getDiscounts($master_id);
            }

            // Downloads
            if (!isset($override['product_download'])) {
                $product_data['product_download'] = $this->model_sync_product->getDownloads($master_id);
            }

            // Filters
            if (!isset($override['product_filter'])) {
                $product_data['product_filter'] = $this->model_sync_product->getFilters($master_id);
            }

            // Images
            if (!isset($override['product_image'])) {
                $product_data['product_image'] = $this->model_sync_product->getImages($master_id);
            }

            // Layouts
            if (!isset($override['product_layout'])) {
                $product_data['product_layout'] = $this->model_sync_product->getLayouts($master_id);
            }
            ...
            if (!isset($override['product_subscription'])) {
                $product_data['product_subscription'] = $this->model_sync_product->getSubscriptions($master_id);
            }

            // Related
            if (!isset($override['product_related'])) {
                $product_data['product_related'] = $this->model_sync_product->getRelated($master_id);
            }

            // Rewards
            if (!isset($override['product_reward'])) {
                $product_data['product_reward'] = $this->model_sync_product->getRewards($master_id);
            }
            ...
            if (!isset($override['product_store'])) {
                $product_data['product_store'] = $this->model_sync_product->getStores($master_id);
            }
        }
        ...
        return $this->model_sync_product->addProduct($product_data);
    }
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->editVariant($master_id, $product_id, $data);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->editVariants($product_id, $data);
     */
    public function editVariants(int $master_id, array $data): void {
        ...
        $products = $this->model_sync_product->getProducts(['filter_master_id' => $master_id]);
        ...
        foreach ($products as $product) {
            ...
            // Descriptions
            $product_descriptions = $this->model_sync_product->getDescriptions($product['product_id']);
            ...
            // Attributes
            if (isset($override['product_attribute'])) {
                $product_data['product_attribute'] = $this->model_sync_product->getAttributes($product['product_id']);
            }

            // Category
            if (isset($override['product_category'])) {
                $product_data['product_category'] = $this->model_sync_product->getCategories($product['product_id']);
            }

            // Discounts
            if (isset($override['product_discount'])) {
                $product_data['product_discount'] = $this->model_sync_product->getDiscounts($product['product_id']);
            }

            // Downloads
            if (isset($override['product_download'])) {
                $product_data['product_download'] = $this->model_sync_product->getDownloads($product['product_id']);
            }

            // Filters
            if (isset($override['product_filter'])) {
                $product_data['product_filter'] = $this->model_sync_product->getFilters($product['product_id']);
            }

            // Images
            if (isset($override['product_image'])) {
                $product_data['product_image'] = $this->model_sync_product->getImages($product['product_id']);
            }

            // Layouts
            if (isset($override['product_layout'])) {
                $product_data['product_layout'] = $this->model_sync_product->getLayouts($product['product_id']);
            }

            // Subscription
            if (isset($override['product_subscription'])) {
                $product_data['product_subscription'] = $this->model_sync_product->getSubscriptions($product['product_id']);
            }

            // Related
            if (isset($override['product_related'])) {
                $product_data['product_related'] = $this->model_sync_product->getRelated($product['product_id']);
            }

            // Rewards
            if (isset($override['product_reward'])) {
                $product_data['product_reward'] = $this->model_sync_product->getRewards($product['product_id']);
            }

            // SEO
            $product_data['product_seo_url'] = $this->model_sync_product->getSeoUrls($product['product_id']);

            // Stores
            if (isset($override['product_store'])) {
                $product_data['product_store'] = $this->model_sync_product->getStores($product['product_id']);
            }
            ...
            $this->model_sync_product->editProduct($product['product_id'], $product_data);
        }
    }
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->editRating($result['product_id'], $this->model_sync_review->getRating($product_id));
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $results = $this->model_sync_product->getProducts($filter_data);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_total = $this->model_sync_product->getTotalProducts();
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_total = $this->model_sync_product->getTotalProductsByManufacturerId($manufacturer_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_total = $this->model_sync_product->getTotalProductsByTaxClassId($tax_class_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_total = $this->model_sync_product->getTotalProductsByStockStatusId($stock_status_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_total = $this->model_sync_product->getTotalProductsByWeightClassId($weight_class_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_total = $this->model_sync_product->getTotalProductsByLengthClassId($length_class_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->addDescription($product_id, $language_id, $product_data);
     */
    public function addDescription(int $product_id, int $language_id, array $data): void {
        ...
        $this->db->query("INSERT INTO `" . DB_PREFIX . "product_description` SET `product_id` = '" . (int)$product_id . "', `language_id` = '" . (int)$language_id . "', `name` = '" . $this->db->escape($data['name']) . "', `description` = '" . $description . "', `tag` = '" . $this->db->escape($data['tag']) . "', `meta_title` = '" . $this->db->escape($data['meta_title']) . "', `meta_description` = '" . $this->db->escape($data['meta_description']) . "', `meta_keyword` = '" . $this->db->escape($data['meta_keyword']) . "'");
    }
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteDescriptionsByLanguageId($language_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_description = $this->model_sync_product->getDescriptions($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $results = $this->model_sync_product->getDescriptionsByLanguageId($language_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $results = $this->model_sync_product->addCode($product_id, $data);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $results = $this->model_sync_product->deleteCodes($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $results = $this->model_sync_product->getCodes($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteCategoriesByCategoryId($category_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $categories = $this->model_sync_product->getCategories($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteFiltersByFilterId($filter_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $filters = $this->model_sync_product->getFilters($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->addAttribute($product_id, $attribute_id, $language_id, $product_data);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteAttributes($product_id, $attribute_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteAttributesByLanguageId($language_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_attributes = $this->model_sync_product->getAttributes($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $results = $this->model_sync_product->getAttributesByLanguageId($language_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_total = $this->model_sync_product->getTotalAttributesByAttributeId($attribute_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->addOption($product_id, $product_data);
     */
    public function addOption(int $product_id, array $data): int {
        ...
        if (isset($data['product_option_value'])) {
            foreach ($data['product_option_value'] as $product_option_value) {
                $this->model_sync_product->addOptionValue($product_id, $product_option_id, $data['option_id'], $product_option_value);
            }
        }
    ...
    public function deleteOptions(int $product_id): void {
        ...
        $this->model_sync_product->deleteOptionValues($product_id);
    }
     ...
     * $this->load->model('catalog/product');
     *
     * $product_option_info = $this->model_sync_product->getOption($product_id, $product_option_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_options = $this->model_sync_product->getOptions($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_total = $this->model_sync_product->getTotalOptionsByOptionId($option_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->addOptionValue($product_id, $product_option_id, $option_id, $product_option_value);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_option_value_info = $this->model_sync_product->getOptionValue($product_id, $product_option_value_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_option_values = $this->model_sync_product->getOptionValuesByOptionId($option_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $option_value = $this->model_sync_product->getTotalOptionValuesByOptionValueId($option_value_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->addImage($product_id, $product_data);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_images = $this->model_sync_product->getImages($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->addDiscount($product_id, $product_data);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteDiscountsByCustomerGroupId($customer_group_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_discounts = $this->model_sync_product->getDiscounts($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->addReward($product_id, $customer_group_id, $product_data);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteRewardsByCustomerGroupId($customer_group_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_reward = $this->model_sync_product->getRewards($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteDownloadsByDownloadId($download_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_download = $this->model_sync_product->getDownloads($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_total = $this->model_sync_product->getTotalDownloadsByDownloadId($download_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteStoresByStoreId($store_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_store = $this->model_sync_product->getStores($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteLayoutsByLayoutId($layout_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteLayoutsByStoreId($store_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_seo_url = $this->model_sync_product->getSeoUrls($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_layout = $this->model_sync_product->getLayouts($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_total = $this->model_sync_product->getTotalLayoutsByLayoutId($layout_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_relateds = $this->model_sync_product->getRelated($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->addSubscription($product_id, $product_data);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_subscription_info = $this->model_sync_product->getSubscription($product_id, $subscription_plan_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_subscriptions = $this->model_sync_product->getSubscriptions($product_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $this->model_sync_product->deleteSubscriptionsBySubscriptionPlanId($subscription_plan_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $product_total = $this->model_sync_product->getTotalSubscriptionsBySubscriptionPlanId($subscription_plan_id);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $results = $this->model_sync_product->getReports($product_id, $start, $limit);
     */
     ...
     * $this->load->model('catalog/product');
     *
     * $report_total = $this->model_sync_product->getTotalReports($product_id);
     */
