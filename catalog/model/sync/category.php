...
class Category extends \Opencart\System\Engine\Model {
    ...
    public function getCategoryByCategoryId(int $category_id): array {
        $category_data = [];
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "category` `c` WHERE `c`.`category_id` = '" . $category_id . "';");
     ...
     *
     * $category_id = $this->model_sync_category->addCategory($category_data);
     */
    public function addCategory(array $data): int {
        $this->db->query("INSERT INTO `" . DB_PREFIX . "category` SET `category_id` = '" . (int)$data['category_id'] . "',`image` = '" . $this->db->escape((string)$data['image']) . "', `parent_id` = '" . (int)$data['parent_id'] . "', `sort_order` = '" . (int)$data['sort_order'] . "', `status` = '" . (bool)($data['status'] ?? 0) . "'");

        $category_id = $data['category_id'];

        foreach ($data['category_description'] as $language_id => $category_description) {
            $this->model_sync_category->addDescription($category_id, $language_id, $category_description);
        }

        $level = 0;

        // MySQL Hierarchical Data Closure Table Pattern
        $results = $this->model_sync_category->getPaths($data['parent_id']);

        foreach ($results as $result) {
            $this->model_sync_category->addPath($category_id, $result['path_id'], $level);

            $level++;
        }

        $this->model_sync_category->addPath($category_id, $category_id, $level);

        if (isset($data['category_filter'])) {
            foreach ($data['category_filter'] as $filter_id) {
                $this->model_sync_category->addFilter($category_id, $filter_id);
            }
        }
        ...
        if (isset($data['category_store'])) {
            foreach ($stores as $store) {
                if (!in_array($store['store_id'], $data['category_store'])) {
                    array_push($data['category_store'], $store['store_id']);
                }
            }
            foreach ($data['category_store'] as $store_id) {
                $this->model_sync_category->addStore($category_id, $store_id);
            }
        }

        // Seo urls on categories need to be done differently to they include the full keyword path
        $parent_path = $this->model_sync_category->getPath($data['parent_id']);
        ...
        foreach ($data['category_seo_url'] as $store_id => $language) {
            foreach ($language as $language_id => $keyword) {
                ...
                if (in_array($language_id, $language_list)) {
                    ...
                    foreach ($stores as $store) {
                        $this->model_design_seo_url->addSeoUrl('path', $path, $keyword, $store['store_id'], $language_id);
                    }
                }
            }
        }
        ...
        if (isset($data['category_layout'])) {
            foreach ($data['category_layout'] as $store_id => $layout_id) {
                if ($layout_id) {
                    $this->model_sync_category->addLayout($category_id, $store_id, $layout_id);
                }
            }
        }
     ...
     * $this->load->model('catalog/category');
     *
     * $this->model_sync_category->editCategory($category_id, $category_data);
     */
    public function editCategory(int $category_id, array $data): void {
        ...
        $this->model_sync_category->deleteDescriptions($category_id);
        ...
        // Path
        $path_old = $this->model_sync_category->getPath($category_id);

        $path_parent = '';

        if (!empty($data['parent_id'])) {
            $path_parent = $this->model_sync_category->getPath($data['parent_id']);
        }

        $path_new = $path_parent ? implode('_', [$path_parent, $category_id]) : $category_id;

        // Delete the category paths
        $this->model_sync_category->deletePaths($category_id);

        // Delete paths
        $results = $this->model_sync_category->getPathsByPathId($category_id);
        ...
        $results = $this->model_sync_category->getPaths($category_id);
        ...
        foreach ($paths as $path_id) {
            $this->model_sync_category->addPath($category_id, $path_id, $level);
        ...
        $this->model_sync_category->repairCategories($category_id);
        ...
        foreach ($data['category_seo_url'] as $store_id => $language) {
            foreach ($language as $language_id => $keyword) {
                if ($path_parent && isset($keywords_parent[$store_id][$language_id])) {
            ...
            foreach ($language as $language_id => $paths) {
                if (in_array($language_id, $language_list)) {
                    foreach ($paths as $value => $keyword) {
                        ...
                        foreach ($stores as $store) {
                            $this->model_design_seo_url->addSeoUrl('path', $value, $keyword, $store['store_id'], $language_id);
                        }
                    }
                }
            }
        ...
        // Filters
        $this->model_sync_category->deleteFilters($category_id);
        ...
        // Stores
        $this->model_sync_category->deleteStores($category_id);
        ...
        // Layouts
        $this->model_sync_category->deleteLayouts($category_id);
     ...
     * $this->load->model('catalog/category');
     *
     * $this->model_sync_category->deleteCategory($category_id);
     */
    public function deleteCategory(int $category_id): void {
        ...
        $path = $this->model_sync_category->getPath($category_id);
        ...
        foreach ($results as $result) {
            if ($result['category_id'] != $category_id) {
                $this->model_sync_category->deleteCategory($result['category_id']);
            }
        }
     ...
     * $this->load->model('catalog/category');
     *
     * $this->model_sync_category->repairCategories();
     */
    public function repairCategories(int $parent_id = 0): void {
        ...
        foreach ($query->rows as $category) {
            // Delete the path below the current one
            $this->model_sync_category->deletePaths($category['category_id']);

            // Fix for records with no paths
            $level = 0;

            $paths = $this->model_sync_category->getPaths($parent_id);

            foreach ($paths as $path) {
                $this->model_sync_category->addPath($category['category_id'], $path['path_id'], $level);

                $level++;
            }

            $this->model_sync_category->addPath($category['category_id'], $category['category_id'], $level);
            $this->model_sync_category->repairCategories($category['category_id']);
        }
    }
     ...
     * $this->load->model('catalog/category');
     *
     * $category_info = $this->model_sync_category->getCategory($category_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $results = $this->model_sync_category->getCategories();
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $category_total = $this->model_sync_category->getTotalCategories($filter_data);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $this->model_sync_category->addDescription($category_id, $language_id, $category_data);
     */
    public function addDescription(int $category_id, int $language_id, array $data): void {
        ...
        $this->db->query("INSERT INTO `" . DB_PREFIX . "category_description` SET `category_id` = '" . (int)$category_id . "', `language_id` = '" . (int)$language_id . "', `name` = '" . $this->db->escape($data['name']) . "', `description` = '" . $description . "', `meta_title` = '" . $this->db->escape($data['meta_title']) . "', `meta_description` = '" . $this->db->escape($data['meta_description']) . "', `meta_keyword` = '" . $this->db->escape($data['meta_keyword']) . "'");
    }
     ...
     * $this->load->model('catalog/category');
     *
     * $this->model_sync_category->deleteDescriptionsByLanguageId($language_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $category_description = $this->model_sync_category->getDescriptions($category_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $results = $this->model_sync_category->getDescriptionsByLanguageId($language_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $this->model_sync_category->deletePathsByLevel($category_id, $level);
     */
    ...
    public function getPath(int $category_id): string {
        return implode('_', array_column($this->model_sync_category->getPaths($category_id), 'path_id'));
    }
     ...
     * $this->load->model('catalog/category');
     *
     * $results = $this->model_sync_category->getPaths($parent_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $this->model_sync_category->deleteFiltersByFilterId($filter_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $filters = $this->model_sync_category->getFilters($category_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $this->model_sync_category->deleteStoresByStoreId($store_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $category_store = $this->model_sync_category->getStores($category_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $this->model_sync_category->deleteLayoutsByLayoutId($layout_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $this->model_sync_category->deleteLayoutsByStoreId($store_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $category_layout = $this->model_sync_category->getLayouts($category_id);
     */
     ...
     * $this->load->model('catalog/category');
     *
     * $category_total = $this->model_sync_category->getTotalLayoutsByLayoutId($layout_id);
     */
